...
with t1 as
	(select pt.patient_id,pt.event_name,ec.category,pt.physician_id,ps.speciality
	...
	t2 as 
	(select physician_id, speciality
	from t1
	where category='Procedure'
		and category<>'Prescription'),
	t3 as
	(select * 
	from t2
	group by physician_id, speciality)
